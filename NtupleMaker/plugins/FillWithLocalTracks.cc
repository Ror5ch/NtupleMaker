...
void NtupleMaker::FillWithLocalTracks(const edm::Event& iEvent)
{
    edm::Handle<std::vector<CTPPSLocalTrackLite>> local_tracks;
    iEvent.getByToken(local_tracks_token_, local_tracks);
    ...
    for (const auto& local_track : *local_tracks)
    {
        ...
        // These variables are not available in 9_4_11 branch
